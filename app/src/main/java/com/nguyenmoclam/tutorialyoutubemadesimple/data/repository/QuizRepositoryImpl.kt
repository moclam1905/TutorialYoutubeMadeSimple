...
class QuizRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTranscriptByQuizId(quizId: Long): Transcript? {
        return transcriptDao.getTranscriptForQuiz(quizId).first()?.let { TranscriptMapper.toDomain(it) }
    }
    ...
    override suspend fun insertTopics(topics: List<Topic>, quizId: Long) {
        ...
        topics.forEachIndexed { index, topic ->
            ...
            if (questions.isNotEmpty()) {
                contentQuestionDao.insertQuestions(questions.map { ContentQuestionMapper.toEntity(it, topicId) })
            }
        }
    }
