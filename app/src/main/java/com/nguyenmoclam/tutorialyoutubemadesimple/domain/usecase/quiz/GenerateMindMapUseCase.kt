...
class GenerateMindMapUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        videoTitle: String?
    ): MindMapResult = withContext(Dispatchers.IO) {
        try {
            // Prompt 1: Extract key points
            val keyPoints =
                llmProcessor.extractKeyPointsForMindMap(transcriptContent, transcriptLanguage)
                    .takeIf { it.isNotEmpty() } ?: return@withContext MindMapResult(
                    "",
            ...
            val code = llmProcessor.generateMermaidMindMapCode(
                keyPoints,
                videoTitle ?: "",
                transcriptLanguage
            )
                .takeIf { it.isNotBlank() } ?: return@withContext MindMapResult(
                "",
